...
<?php foreach ($gridFields as $fieldlist) { ?>
                    <th
                        data-column-id="<?= $fieldlist['id'] ?>"
                        data-width="<?= (empty($fieldlist['width']) ? ('') : ($fieldlist['width'])) ?>"
                        data-type="<?= (empty($fieldlist['type']) ? ('string') : ($fieldlist['type'])) ?>"
                        data-formatter="<?= (empty($fieldlist['formatter']) ? ('') : ($fieldlist['formatter'])) ?>"
                    ><?= (empty($fieldlist['heading']) ? ('') : ($fieldlist['heading'])) ?></th>
<?php } ?>
...
<?= $this->partial('layout_partials/base_dialog', ['fields' => $formDialogFilterRule, 'id' => 'DialogFilterRule', 'label' => $lang->_('Edit rule')]) ?>
